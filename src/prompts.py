...
2. Determine if the email is:
   - **Sendable** (`send: true`): The email is reasonable and addresses the customer's inquiry. Default to approving unless there are serious issues.
   - **Not Sendable** (`send: false`): Only use when the email is clearly unprofessional, factually wrong, fails to address the customer's question, or would harm customer relations.
3. **Be lenient**: Prefer approving drafts. Only set `send: false` for significant problems such as: incorrect product information, rude tone, completely off-topic response, or obvious grammatical errors that would embarrass the company.
4. Provide actionable feedback for the writer agent only when the email is deemed "not sendable."
...
* Default to `send: true` when the email is decent and professional.
* Only reject when there are clear, serious issues that would negatively impact the customer.
* Minor improvements do NOT warrant rejection—approve and let it through.
"""