...
class GmailToolsClass:
    ...
    def get_or_create_label(self, label_name):
        """
        Get label ID by name, or create the label if it doesn't exist.
    ...
    def apply_label_to_message(self, message_id, label_id):
        """Apply a label to a message (and thus its thread in Gmail UI)."""
        ...
        try:
            ...
            print(f"An error occurred while applying label to message: {error}")
            return False

    def apply_ai_handled_label(self, message_id):
        """Apply 'AI Handled' label to threads where a draft reply was created."""
        label_id = self.get_or_create_label("AI Handled")
        return self.apply_label_to_message(message_id, label_id)

    def apply_review_later_label(self, message_id):
        """Apply 'Review Later' label to unrelated emails (not spam, user can reply when ready)."""
        label_id = self.get_or_create_label("Review Later")
        return self.apply_label_to_message(message_id, label_id)
    ...
    def _should_skip_email(self, email_info):
        ...
        if not my_email:
            return False
        return my_email in email_info.get('sender', '')
